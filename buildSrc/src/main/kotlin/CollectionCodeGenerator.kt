...
abstract class CollectionCodeGenerator : DefaultTask() {
    ...
    private fun scanClassFile(classFile: File): EntityMetadata? {
        ...
        var collectionName = ""
        var createAuth = mutableListOf<String>()
        var readAuth = mutableListOf<String>()
        var updateAuth = mutableListOf<String>()
        var deleteAuth = mutableListOf<String>()
        var className = ""
        var packageName = ""
        var idFieldName = ""
                    ...
                    "Lbr/com/firstsoft/shapr/cms/annotation/Collection;" -> {
                        ...
                        return object : AnnotationVisitor(Opcodes.ASM9) {
                            ...
                            override fun visitArray(name: String?): AnnotationVisitor {
                                return object : AnnotationVisitor(Opcodes.ASM9) {
                                    override fun visit(arrayName: String?, value: Any?) {
                                        val strValue = value as? String ?: return
                                        when (name) {
                                            "createAuth" -> createAuth.add(strValue)
                                            "readAuth" -> readAuth.add(strValue)
                                            "updateAuth" -> updateAuth.add(strValue)
                                            "deleteAuth" -> deleteAuth.add(strValue)
                                        }
                                    }
                                }
                            }
        ...
        return if (hasEntityAnnotation && hasCollectionAnnotation) {
            EntityMetadata(
                ...
                className = className,
                idType = idFieldType,
                path = collectionName.ifEmpty { pluralize(className).lowercase() },
                createRoles = normalizeRoles(createAuth),
                readRoles = normalizeRoles(readAuth),
                updateRoles = normalizeRoles(updateAuth),
                deleteRoles = normalizeRoles(deleteAuth)
            )
        } else null
    }

    private fun normalizeRoles(roles: List<String>): List<String> =
        if (roles.isEmpty()) listOf("admin") else roles
    ...
    private fun generateController(entity: EntityMetadata, outputDir: File) {
        ...
        val content = """
            ...
            |import br.com.firstsoft.shapr.cms.auth.AuthUtil
            ...
            |) {
            |    @GetMapping
            |    fun list(): List<${entity.className}> {
            |        AuthUtil.requireRole(listOf(${formatRoles(entity.readRoles)}))
            ...
            |        AuthUtil.requireRole(listOf(${formatRoles(entity.createRoles)}))
            ...
            |        AuthUtil.requireRole(listOf(${formatRoles(entity.updateRoles)}))
            ...
            |        AuthUtil.requireRole(listOf(${formatRoles(entity.deleteRoles)}))
            |        return if (repository.existsById(id)) {
